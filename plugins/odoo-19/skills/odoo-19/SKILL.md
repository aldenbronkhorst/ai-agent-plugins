...
- For direct JSON-2 calls, prefer the bundled `scripts/odoo_json2.py` client over rebuilding HTTP and authentication code when it fits the request. It accepts any Odoo model, method, and JSON-object body and returns Odoo's response unchanged; it is a transport helper, not a restricted operation layer. Use `scripts/credential-contract.json` with a compatible credential provider when credentials must be discovered and injected. The client also works independently with `ODOO_URL`, `ODOO_API_KEY`, and optional `ODOO_DATABASE` supplied securely by any method. If the helper is unsuitable or fails, use the documented JSON-2 request structure directly or choose another appropriate access method rather than treating the helper as a gate.
- Resolve bundled paths relative to this skill directory. The direct invocation is `python3 scripts/odoo_json2.py <MODEL> <METHOD> --body '<JSON_OBJECT>'` on macOS/Linux or `py -3 scripts/odoo_json2.py <MODEL> <METHOD> --body '<JSON_OBJECT>'` on Windows; use `--body-file <PATH>` or `--body-file -` for a file or standard input. The syntax is complete, so do not run a help command first.
