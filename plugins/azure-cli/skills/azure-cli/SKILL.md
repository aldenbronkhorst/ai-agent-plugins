...
- Avoid changing a shared default underneath another task. Specify `--subscription` where supported; use reusable device-local `AZURE_CONFIG_DIR` profiles for concurrent identities or clouds, and serialize operations that would update the same profile's token cache. Do not create a new empty profile for every task or copy token caches between profiles: select the existing intended profile so its sign-in persists.
