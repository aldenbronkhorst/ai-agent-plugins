...
- The helper uses pinned official `@azure/msal-node` and `@azure/msal-node-extensions` packages. It installs them into its user-state directory on first use and relies on DPAPI on Windows, Keychain on macOS, or LibSecret on Linux; it never enables a plaintext token-cache fallback.
...
- If Node.js 20 or later or the pinned MSAL packages are absent, install them in the current user's scope and retry instead of switching to browser automation. Do not substitute Graph PowerShell authentication for this helper on Windows.
