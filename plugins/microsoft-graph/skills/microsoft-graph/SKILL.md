...
- On initial account setup, aim for the maximum compatible delegated access Microsoft will accept for that specific account and tenant unless the user requests narrower access. Determine the scope set at runtime from current Microsoft Graph permission information; cover all available read, write, shared-resource, administration, Intune, security, compliance, reporting, Outlook, files, SharePoint, Teams, directory, identity, and collaboration capabilities. Prefer a broader write or management permission when it subsumes narrower read permissions.
- Maximum access does not mean submitting every published scope blindly. Exclude application-only permissions, permissions incompatible with the account type or tenant, obsolete permissions, resource-specific permissions that require separate setup, and redundant scopes. Request the largest valid compatible set in one consent. If Microsoft rejects the combination, identify and remove only the invalid or incompatible permissions instead of falling back to minimal access. Warn once that maximum delegated access is intentionally powerful and may require administrator approval.
- After authentication, compare the granted scopes with the valid requested set. Do not report broad setup as complete while compatible requested permissions are missing; identify permissions that require administrator consent or are unavailable to that account. Add another consent later only for a capability Microsoft could not include during setup or introduced afterward.
