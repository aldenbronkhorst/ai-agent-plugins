...
- Use the official Microsoft Graph PowerShell SDK in PowerShell 7 for terminal access. If it is absent, install the required modules for the current user. Use `Connect-MgGraph -UseDeviceAuthentication` for sign-in and Graph cmdlets or `Invoke-MgGraphRequest` for operations. Do not introduce a custom authentication runtime.
- On Windows, use `Microsoft.Graph.Authentication` 2.33.0 for device-code authentication and import that exact version in a clean PowerShell 7 process. Versions 2.34 and later forced WAM and are known to complete browser consent while failing or repeatedly prompting when the Graph identity differs from the Windows identity. Keep any newer version installed side by side for unrelated work; do not load it for this device-code session. On macOS and Linux, use the current compatible SDK unless a confirmed platform-specific regression requires otherwise.
...
- On first use of an account, show the device code once and let that same `Connect-MgGraph` process finish. Browser success is only consent confirmation: consider sign-in complete only after that command returns, `Get-MgContext` identifies the intended account, and a simple `GET /v1.0/me` succeeds in the same PowerShell process. Do not keep issuing codes through a known-broken SDK version; restart cleanly with the Windows compatibility version above. Reuse the authenticated context and its refresh behavior on later requests.
