...
- After setup, verify the credential-storage source using `gh auth status` without token-showing options. `gh` can fall back to a plaintext file when the OS credential store is unavailable even without `--insecure-storage`; do not report that as secure storage. Repair the credential-store prerequisite before completing setup, or use an explicitly approved secure provider. Never display the credential file to diagnose this.
...
- `gh auth switch` changes a shared default, not a task-local identity. For concurrent accounts, reuse separate account contexts or serialize account selection and commands rather than switching underneath another task. Token environment variables can override stored logins (`GH_TOKEN`/`GITHUB_TOKEN`, or their enterprise equivalents); check their presence and the effective identity without printing values. A reusable `GH_CONFIG_DIR` separates CLI configuration, but verify the effective account and storage rather than assuming it isolates every OS credential-store entry.
